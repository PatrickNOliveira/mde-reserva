...
class PedidoController extends Controller
{
    ...
    public function uploadImage(Request $request)
    {
        ...
        if ($request->hasFile('logo')) {
            ...
            $filename = uniqid() . '_' . time() . '.' . $logo->getClientOriginalExtension();

            // Armazena o arquivo no diretório especificado
            Storage::putFileAs($directory, $logo, $filename);

            // Retorna uma mensagem de sucesso ou o caminho do arquivo se necessário
    ...
    {
        if ($item == null)
            return response()->json(PedidoViewModel::getCarrinhoSuite($id, $suite_id));
    ...
    {
        if ($item == null)
            return response()->json(PedidoViewModel::getCarrinhoMesa($id, $mesa));
